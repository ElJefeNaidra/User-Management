...
namespace UserManagement.Features.Administration.NonMatchingLogin
{
    ...
    {
        private const string ReturnUrl = "/NonMatchingLoginSimpleSearch/GetView";
        ...
        public NonMatchingLoginController(DBContext dbContext, ResourceRepository resourceRepo, IHttpContextAccessor accessor)
        {
            _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
            _resourceRepo = resourceRepo ?? throw new ArgumentNullException(nameof(resourceRepo));
            _accessor = accessor ?? throw new ArgumentNullException(nameof(accessor));
        }
        private void SetupModel(NonMatchingLoginModel model, CrudOp operation)
        {
