...
namespace UserManagement.Features.CommonRegistry.Language
{
    public partial class LanguageController : Controller
    {
        ...
        [ChildAction("Nomenclatures.Language.Disable", "Language - Disable", "Gjuha - Deaktivizim", "Jezik - Deaktiviranje")]
        [HttpGet]
        ...
        [ChildAction("Nomenclatures.Language.Disable", "Language - Disable", "Gjuha - Deaktivizim", "Jezik - Deaktiviranje")]
        [HttpPost]
        public async Task<IActionResult> Disable(LanguageModel model)
        {
            SetupModel(model, CrudOp.Disable);
            ...
            {
                var (ResponseInfo, Data) = await _dbContext.UpdateAsync<LanguageModel>(SPName, model);

                if (ResponseInfo.HasError || Data == null)
                {
                    TempData["ErrorMessages"] = ResponseInfo.ErrorMessage;
                    return RedirectToAction("Read", new { Id = model.IdLanguageId, _RowGuid = model._RowGuid });
                }
                else
                {
