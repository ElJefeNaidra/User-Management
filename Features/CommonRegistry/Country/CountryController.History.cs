...
namespace UserManagement.Features.CommonRegistry.Country
{
    ...
    {
        #region HISTORY
        private async Task GetHistoryGrid(int Id, CountryModel model)
        {
            var filterParameters = new List<KeyValuePair<string, object>>()
            {
                // Add parameters if you please
                new KeyValuePair<string, object>("IdCountry", Id)
            };
        ...
        [ChildAction("CommonRegistry.Country.ReadHistory", "Country - History", "Shteti - Historiku", "Drzava - Istorija")]
        [HttpGet]
