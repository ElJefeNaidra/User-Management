...
public class RequestAuthorisationFilter : ActionFilterAttribute
{
    ...
    public RequestAuthorisationFilter(IServiceProvider serviceProvider, IUserAgentService userAgentService)
    {
    ...
    public override void OnActionExecuting(ActionExecutingContext context)
    {
        ...
        else
        {
            ...
            #region Permissions / Authorisations check
            string idAuthorisation = null;
            DataTable dataTable = session.GetObjectFromJson<DataTable>("Authorisations");

            string searchExpression = $"Url = '{ControllerAction}'";
            DataRow[] foundRows = dataTable.Select(searchExpression);

            if (foundRows.Length == 0)
            {
                // DENY ACCESS
                context.Result = new RedirectToActionResult("AccessDenied", "UserAccessDenied", null);
                return;
            }
            else
            {
                // Since there's at most one row, we can directly access the first element.
                idAuthorisation = foundRows[0]["IdAuthorisation"].ToString();
            }
            #endregion
            ...
            GlobalAccessTrackingModel model = new()
            {
                ...
                IdAuthorisation = Convert.ToInt32(idAuthorisation),
                SPExecuted = null,
